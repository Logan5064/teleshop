...
const ProductModules: React.FC<ProductModulesProps> = ({ 
  productForm, 
  setProductForm, 
  generateId 
}) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {/* Размеры одежды */}
        {productForm.modules?.clothingSizes && (
          <div className="bg-white/95 backdrop-blur-sm rounded-2xl p-6 border border-gray-300/60 shadow-sm">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-8 h-8 bg-blue-100/80 rounded-lg flex items-center justify-center border border-blue-300/50">
                <RectangleStackIcon className="w-5 h-5 text-blue-600" />
              </div>
              <h4 className="font-medium text-gray-900 leading-none">Размеры одежды</h4>
            </div>
            <div className="flex flex-wrap gap-2">
              {['XS', 'S', 'M', 'L', 'XL', 'XXL'].map((size) => (
                <button
                  key={size}
                  onClick={() => {
                    const currentSizes = productForm.clothingSizes || [];
                  ...
                  className={`px-4 py-2 rounded-lg border-2 text-sm font-medium transition-all ${
                    (productForm.clothingSizes || []).includes(size)
        ...
        {/* Размеры обуви */}
        {productForm.modules?.shoeSizes && (
          <div className="bg-white/95 backdrop-blur-sm rounded-2xl p-6 border border-gray-300/60 shadow-sm">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-8 h-8 bg-orange-100/80 rounded-lg flex items-center justify-center border border-orange-300/50">
                <Squares2X2Icon className="w-5 h-5 text-orange-600" />
              </div>
              <h4 className="font-medium text-gray-900 leading-none">Размеры обуви</h4>
            </div>
            <div className="flex flex-wrap gap-2">
              {Array.from({length: 12}, (_, i) => (i + 35)).map((size) => (
                <button
                  key={size}
                  onClick={() => {
                    const currentSizes = productForm.shoeSizes || [];
                    const isSelected = currentSizes.includes(size);
                    const newSizes = isSelected 
                  ...
                  className={`px-3 py-2 rounded-lg border-2 text-sm font-medium transition-all ${
                    (productForm.shoeSizes || []).includes(size)
        ...
        {productForm.modules?.materials && (
          <div className="bg-white/95 backdrop-blur-sm rounded-2xl p-6 border border-gray-300/60 shadow-sm">
            ...
            <div className="flex flex-wrap gap-2">
              {[
                'Хлопок', 'Полиэстер', 'Кожа', 'Замша', 'Дерево', 
                'Металл', 'Пластик', 'Стекло', 'Керамика', 'Резина'
              ].map((material) => (
                <button
                  key={material}
                  onClick={() => {
                    const isSelected = productForm.material === material;
                    setProductForm((prev: any) => ({ 
                      ...prev, 
                      material: isSelected ? '' : material 
                    }));
                  }}
                  className={`px-4 py-2 rounded-lg border-2 text-sm font-medium transition-all ${
