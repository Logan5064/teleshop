...
export interface BotUserTracking {
  telegram_user_id: number
  shop_id: number
  username?: string
  first_name?: string
...
export interface BotActivityTracking {
  user_id: number
  shop_id: number
  activity_type: string
  description?: string
...
export const analyticsApi = {
  ...
  trackUser: async (data: BotUserTracking): Promise<void> => {
    const response = await api.post('/secure/analytics/track-user', data)
    return response.data
  },

  // Отслеживание активности
  trackActivity: async (data: BotActivityTracking): Promise<void> => {
